...
import { fetchVillages } from '@/lib/supabase/queries';
import type { DbVillage } from '@/lib/supabase/types';
...
const locationColors: Record<string, { bg: string; border: string; dot: string }> = {
...
const defaultColors = { bg: 'bg-slate-50', border: 'border-slate-100', dot: 'bg-slate-400' };

export default function Dashboard() {
  const { orders, lots, inspections, transactions, setActiveTab, selectedLocation } = useApp();

  // ─── Supabase: fetch villages for Location Summary ────────────────────────
  // We use villages from DB if available; otherwise fall back to labLocations.
  // The display metadata (focalPoint, approver) still comes from labLocations since
  // these fields don't exist in the `villages` table yet.
  const [villages, setVillages] = useState<DbVillage[]>([]);
  useEffect(() => {
    fetchVillages().then(({ data }) => {
      if (data && data.length > 0) setVillages(data);
      // Silently fall back to labLocations if DB returns empty or errors
    });
  }, []);

  // Resolved location display list: prefer DB villages, fallback to mock labLocations
  const locationDisplayList = villages.length > 0
    ? villages.map(v => ({
        id: v.id,
        name: v.name as Location,
        code: v.code,
        focalPoint: labLocations.find(l => l.name === v.name)?.focalPoint ?? '—',
        approver: labLocations.find(l => l.name === v.name)?.approver ?? '—',
      }))
    : labLocations.map(l => ({ id: l.id, name: l.name, code: l.code, focalPoint: l.focalPoint, approver: l.approver }));

  // ─── Derived data (from AppContext) ───────────────────────────────────────
  ...
  return (
    <div className="fade-in space-y-6">
      ...
      {/* Location Summary Cards — using Supabase villages (or mock fallback) */}
        ...
        <div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {locationDisplayList.map(lab => {
              ...
              const colors = locationColors[lab.name] ?? defaultColors;
              return (
