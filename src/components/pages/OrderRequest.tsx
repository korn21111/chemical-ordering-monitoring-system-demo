...
import { PlusCircle, CheckCircle, FlaskConical, MapPin, Loader2, AlertTriangle } from 'lucide-react';
import clsx from 'clsx';
import { fetchMaterialList } from '@/lib/supabase/queries';
import type { DbMaterialListItem } from '@/lib/supabase/types';
...
// Normalise a DB material item to the same shape as mock Chemical
...
export default function OrderRequest() {
  ...
  useEffect(() => {
    ...
    fetchMaterialList().then(({ data, error }) => {
      if (cancelled) return;
      if (error || !data) {
        // Fallback to mock chemicals list
        setItems(chemicals.map(c => ({
          id: c.id,
          name: c.name,
          casNumber: c.casNumber,
          category: c.category,
          unit: c.unit,
          isPeroxide: c.isPeroxide,
          cylinderId: c.cylinderId,
          minStock: c.minStock,
          isRegulated: c.isRegulated,
        })));
        setItemsError(error ?? 'Could not load item list. Showing demo data.');
      } else if (data.length === 0) {
        // DB empty — fall back to mock
        setItems(chemicals.map(c => ({
          id: c.id, name: c.name, casNumber: c.casNumber,
          category: c.category, unit: c.unit, isPeroxide: c.isPeroxide,
          cylinderId: c.cylinderId, minStock: c.minStock, isRegulated: c.isRegulated,
        })));
      } else {
        setItems(data.map(adaptDbItem));
      }
  ...
  const filteredItems = filterCat === 'All' ? items : items.filter(c => c.category === filterCat);
  const selectedItem = items.find(c => c.id === chemId);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!chemId || !quantity || !purpose || !requester) return;
    const item = items.find(c => c.id === chemId)!;
    addOrder({
      chemicalId: item.id,
    ...
    <div className="fade-in space-y-6">
      ...
      {itemsError && (
        <div className="bg-amber-50 border border-amber-200 text-amber-700 rounded-xl px-4 py-3 text-sm font-medium flex items-center gap-2">
          <AlertTriangle className="w-4 h-4" /> {itemsError}
        </div>
      )}
        ...
        <div className="xl:col-span-2 card">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              {itemsLoading ? (
                <div className="form-input bg-slate-50 flex items-center gap-2 text-slate-400">
            ...
            {selectedItem && (
              <div className="p-3 bg-slate-50 rounded-xl space-y-2 text-xs fade-in">
                ...
                {selectedItem.casNumber !== 'N/A' && (
                  <div className="flex items-center justify-between">
                    <span className="text-slate-500">CAS Number</span>
                    <span className="font-mono font-semibold text-slate-700">{selectedItem.casNumber}</span>
                  </div>
                )}
                {selectedItem.cylinderId && (
                  <div className="flex items-center justify-between">
                    <span className="text-slate-500">Cylinder ID</span>
                    <span className="font-mono font-semibold text-slate-700">{selectedItem.cylinderId}</span>
                  </div>
                )}
                {selectedItem.isPeroxide && (
                ...
                {selectedItem.minStock !== undefined && (
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-semibold text-slate-700">{selectedItem.minStock} {selectedItem.unit}</span>
                  </div>
                )}
                {selectedItem.isRegulated && (
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <div className="form-input bg-slate-50 text-slate-500">{selectedItem?.unit || '—'}</div>
              </div>
            </div>
            ...
            <button type="submit" className="btn-primary w-full justify-center" disabled={itemsLoading}>
