...
import {
  ...
  Clock, X, FileText, Loader2
} from 'lucide-react';
import clsx from 'clsx';
import {
  ChemicalCategory, Location, RegulationType, RegulationStatus,
  RegulatoryRecord, initialRegulatoryRecords
} from '@/lib/mockData';
import { fetchRegulatoryRecords } from '@/lib/supabase/queries';
import type { DbRegulatoryRecord } from '@/lib/supabase/types';
...
// ─── Adapter: DB row → RegulatoryRecord ──────────────────────────────────────
function adaptDbRecord(r: DbRegulatoryRecord): RegulatoryRecord {
  return {
    id: r.id,
    itemName: r.item_name ?? '—',
    itemCode: r.item_code ?? '—',
    category: (r.category ?? 'Chemicals/Reagents') as ChemicalCategory,
    regulationType: (r.regulation_type ?? 'Hazardous') as RegulationType,
    ...
    isControlled: r.is_controlled ?? false,
    linkedLotIds: r.linked_lot_ids ?? [],
  };
}
...
export default function RegulatoryRelated() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    fetchRegulatoryRecords().then(({ data, error: err }) => {
      if (cancelled) return;
      if (err || !data) {
        // Fallback to mock data on error
        setRecords(initialRegulatoryRecords);
        setError(err ?? 'Failed to load regulatory records. Showing demo data.');
      } else if (data.length === 0) {
        // DB empty — use mock data as demo
        setRecords(initialRegulatoryRecords);
      } else {
        setRecords(data.map(adaptDbRecord));
      }
      setLoading(false);
    });
  ...
  }, [records, selectedLocation, filterLocation, filterCategory, filterRegType, searchName, filterStatus]);
    ...
    <div className="fade-in space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="kpi-card">
          ...
          <div>
            <p className="kpi-value">{loading ? '—' : stats.active}</p>
        ...
        <div className="kpi-card">
          ...
          <div>
            <p className="kpi-value">{loading ? '—' : stats.pendingReview}</p>
        ...
        <div className="kpi-card">
          ...
          <div>
            <p className="kpi-value">{loading ? '—' : stats.controlled}</p>
      ...
      <div className="card p-0 overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
          ...
          <span className="text-xs text-slate-400">{loading ? 'Loading…' : `${filtered.length} records`}</span>
        </div>
        <div className="table-container">
          {loading ? (
            <div className="flex items-center justify-center py-16 gap-2 text-slate-400">
              <Loader2 className="w-5 h-5 animate-spin" />
              <span className="text-sm">Loading regulatory records…</span>
            </div>
          ) : (
