...
import {
  ...
  X, Plus, Calendar, MapPin, Loader2
} from 'lucide-react';
import clsx from 'clsx';
import {
  InventoryLot, ShelfLifeExtension, ChemicalCategory, Location,
  initialLots, initialShelfLifeExtensions
} from '@/lib/mockData';
import { fetchItemLots, fetchShelfLifeExtensions } from '@/lib/supabase/queries';
import type { DbItemLot, DbShelfLifeExtension } from '@/lib/supabase/types';
...
// ─── Adapters ─────────────────────────────────────────────────────────────────
function adaptDbLot(r: DbItemLot): InventoryLot {
  return {
    id: r.id,
    ...
    expiryDate: r.expiry_date ?? '9999-12-31',
    receivedBy: r.received_by ?? '—',
    isPeroxide: r.is_peroxide ?? false,
    location: (r.lab_name ?? 'Central Lab') as Location,
  };
}

function adaptDbExtension(r: DbShelfLifeExtension): ShelfLifeExtension {
  return {
    id: r.id,
    lotId: r.lot_id ?? '',
    chemicalName: r.item_name ?? '—',
...
export default function ExtendShelfLife() {
  const { requestExtension, approveExtension, rejectExtension, selectedLocation } = useApp();

  // ─── Supabase data ────────────────────────────────────────────────────────
  const [lots, setLots] = useState<InventoryLot[]>([]);
  const [lotsLoading, setLotsLoading] = useState(true);
  const [extensions, setExtensions] = useState<ShelfLifeExtension[]>([]);
  const [extLoading, setExtLoading] = useState(true);
  const [dataError, setDataError] = useState<string | null>(null);

  useEffect(() => {
    let cancelled = false;
    Promise.all([fetchItemLots(), fetchShelfLifeExtensions()]).then(
      ([lotsResult, extResult]) => {
        if (cancelled) return;

        // Lots
        if (lotsResult.error || !lotsResult.data) {
          setLots(initialLots);
          setDataError('Could not load inventory data from server. Showing demo data.');
        } else {
          setLots(lotsResult.data.length === 0 ? initialLots : lotsResult.data.map(adaptDbLot));
        }
        setLotsLoading(false);

        // Extensions
        if (extResult.error || !extResult.data) {
          setExtensions(initialShelfLifeExtensions);
        } else {
          setExtensions(extResult.data.length === 0 ? initialShelfLifeExtensions : extResult.data.map(adaptDbExtension));
        }
        setExtLoading(false);
      }
    );
  ...
  // Merge local extension state from AppContext with Supabase-loaded extensions
  // (write ops go through AppContext; we display the Supabase-fetched list merged with any local writes)
  const { shelfLifeExtensions: contextExtensions } = useApp();
  const displayExtensions = useMemo(() => {
    // If AppContext has more (local writes), prefer those; otherwise use Supabase data
    return contextExtensions.length > extensions.length ? contextExtensions : extensions;
  }, [contextExtensions, extensions]);

  const filteredExtensions = useMemo(() => {
    const base = activeExtTab === 'Pending'
      ? displayExtensions.filter(e => e.status === 'Pending')
      : displayExtensions;
    return base.sort((a, b) => b.requestDate.localeCompare(a.requestDate));
  }, [displayExtensions, activeExtTab]);

  const pendingCount = displayExtensions.filter(e => e.status === 'Pending').length;
  ...
  return (
    <div className="fade-in space-y-6">
      ...
      {dataError && (
        <div className="bg-amber-50 border border-amber-200 text-amber-700 rounded-xl px-4 py-3 text-sm font-medium flex items-center gap-2">
      ...
      <div className="card p-0 overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
          ...
          <span className="text-xs text-slate-400">{lotsLoading ? 'Loading…' : `${filteredLots.length} lots`}</span>
        </div>
        <div className="table-container">
          {lotsLoading ? (
            <div className="flex items-center justify-center py-16 gap-2 text-slate-400">
              <Loader2 className="w-5 h-5 animate-spin" />
              <span className="text-sm">Loading inventory lots…</span>
            </div>
          ) : (
      ...
      <div className="card p-0 overflow-hidden">
        ...
        {extLoading ? (
          <div className="flex items-center justify-center py-16 gap-2 text-slate-400">
            <Loader2 className="w-5 h-5 animate-spin" />
            <span className="text-sm">Loading extension records…</span>
          </div>
        ) : (
